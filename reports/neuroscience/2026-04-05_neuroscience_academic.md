# DCFN Technical Report: Neuroscience

**How This Analysis Was Conducted**

**Engine:** DCFN v0.3.0-prototype — Living Eden Frameworks LLC
**Date:** April 05, 2026
**Sources:** 2254

---

*This document is the methodology receipt for the accompanying Research Intelligence Article. It documents every parameter, threshold, and result produced by the DCFN pipeline. For findings, interpretation, and gap-bridging proposals, see the Article.*

## 1. Pipeline Summary

| Metric | Value |
|--------|-------|
| Domain | Neuroscience |
| Sources Analyzed | 2254 |
| Graph Nodes | 2267 |
| Graph Edges | 207509 |
| Knowledge Gaps (total) | 2185 |
| Knowledge Gaps (critical) | 0 |
| Research Clusters | 4 |
| Convergence Events | 1 |
| Generated Hypotheses | 5 |
| Cross-Domain Bridges | 0 |
| Golden Path Length | 10 |
| Golden Path Confidence | HIGH |

**Cognitive abilities exercised:** 10/10 — Perception, Generation, Attention, Learning, Memory, Reasoning, Metacognition, Executive Functions, Problem Solving, Social Cognition

## 2. Data Collection

**Total sources:** 2254

**Source APIs:**

| API | Status |
|-----|--------|
| Semantic Scholar | Primary |
| arXiv | Supplementary |
| PubMed | Supplementary |
| OpenAlex | Supplementary |
| GitHub | Supplementary |
| Local Files | If configured |

**Deduplication:** Title-based fuzzy matching (SequenceMatcher ≥ 0.85) across all sources. Sources lacking abstracts excluded.

**Epistemic Grounding Distribution:**

| Status | Count |
|--------|-------|
| Well-grounded (≥0.7) | 1037 |
| Partially grounded (0.4–0.7) | 1129 |
| Weakly grounded (0.2–0.4) | 88 |
| Ungrounded (<0.2) | 0 |
| **Mean grounding score** | **0.749** |

## 3. Source Encoding (QEB)

### 3.1 Method

Sentence-BERT (all-MiniLM-L6-v2) → 384-dimensional semantic embeddings. Gaussian perturbation (σ = 0.1) applied per QECO Patent §6.2.1.

### 3.2 Three-Signal Adaptive Confidence

| Signal | Weight | Description |
|--------|--------|-------------|
| Perturbation Stability | 0.2902 | Mean pairwise cosine across 5 perturbation iterations |
| Vector Concentration | 0.2078 | Energy fraction in top 10% of embedding dimensions |
| Corpus Distinctiveness | 0.502 | Cosine distance from corpus centroid |

**Formula:** Composite = 0.2902 × Stability + 0.2078 × Concentration + 0.502 × Distinctiveness (calibrated)

### 3.3 Calibration State

| Parameter | Value |
|-----------|-------|
| Embedding dim | 384 |
| Perturbation σ | 0.1 |
| LOW threshold | < 0.36505710276783987 (30th percentile) |
| HIGH threshold | ≥ 0.4441114206359239 (80th percentile) |
| Concentration p25/p50/p75 | 0.4235723093152046 / 0.4372384697198868 / 0.4513924643397331 |
| Distinctiveness p25/p50/p75 | 0.41524483263492584 / 0.47721216082572937 / 0.5614220201969147 |
| Stability p25/p50/p75 | 0.19293005567741583 / 0.20612502410618605 / 0.2198386206302052 |

## 4. Concept Graph

### 4.1 Structure

| Metric | Value |
|--------|-------|
| Total Nodes | 2267 |
|   — CONCEPT | 13 |
|   — DOCUMENT | 2254 |
| Total Edges | 207509 |
|   — MIRRORS | 191959 |
|   — EXTENDS | 11424 |
|   — CONTAINS | 3522 |
|   — ENABLES | 569 |
|   — DEPENDS_ON | 35 |
| Density | 0.040395 |
| Connected Components | 71 |

### 4.3 Edge Construction Rules

| Edge Type | Rule |
|-----------|------|
| ENABLES | older → newer via citation (ref list) |
| DEPENDS_ON | newer → older via citation |
| EXTENDS | cosine sim > 0.85 |
| MIRRORS | cosine 0.5–0.85, different fields |
| BRIDGES | cross-domain semantic connection |
| CONTAINS | source → field of study |

**Temporal decay:** confidence(t) = max(0.55, initial × exp(−0.12 × t)), t in months

## 5. CTE Traversal Results

### 5.1 Backward Traversal

**Chains analyzed:** 5

| Source | Chain Depth | Total Drift | Forgotten Ancestors |
|--------|------------|-------------|--------------------|
| Neuroscience | 1 | 0.00 | 0 |
| Contribution of default mode network to game and d | 1 | 0.00 | 1 |
| Sleep Enhances Consolidation of Memory Traces for  | 1 | 0.00 | 1 |
| Temporal Dynamics of Activity in Default Mode Netw | 1 | 0.00 | 1 |
| Novelty modulates human striatal activation and pr | 1 | 0.00 | 1 |

### 5.2 Forward Cascade

**Cascades analyzed:** 5

| Source | Implications | Longest Chain | High Conf | Blocked |
|--------|-------------|--------------|----------|---------|
| Beyond Cognitive Load Theory: Why Learning Ne | 0 | 0 | 0 | 0 |
| Mirror Neurons and Pain: A Scoping Review of  | 0 | 0 | 0 | 0 |
| D-MEM: Dopamine-Gated Agentic Memory via Rewa | 0 | 0 | 0 | 0 |
| Span capacity and age-related differences in  | 0 | 0 | 0 | 0 |
| BDNF and miRNAs in acupuncture-regulated neur | 0 | 0 | 0 | 0 |

### 5.3 Entropy Detection

**Total entropy nodes:** 2185
**Critical (≥7):** 0 | **High (5-6):** 1990 | **Low (<5):** 195

**Issue Distribution:**

| Type | Count |
|------|-------|
| DECAYED | 2185 |
| STALE | 2131 |
| ORPHAN | 69 |

**Complete Entropy Node List:**

| # | Title | Severity | Issues | Bridge Conf | Upstream | Downstream |
|---|-------|----------|--------|------------|----------|------------|
| 1 | Synaptic Plasticity Forms and Functions. | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 2 | 20 years of the default mode network: A revie | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 3 | Review and Classification of Emotion Recognit | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 4 | From Cognitive Load Theory to Collaborative C | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 5 | Casting a Wide Net: Role of Perineuronal Nets | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 6 | Microglia regulation of synaptic plasticity a | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 7 | Rehabilitation robots for the treatment of se | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 8 | Dopamine reward prediction error coding | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 9 | Diverse synaptic plasticity mechanisms orches | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 10 | Neuroplasticity of Language Networks in Aphas | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 11 | Training the Emotional Brain: Improving Affec | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 12 | A Review of Transcranial Magnetic Stimulation | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 13 | Neural functions of calcineurin in synaptic p | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 14 | Neural Correlates of Verbal Working Memory: A | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 15 | The Implications of Microglial Regulation in  | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 16 | Dopamine neurons share common response functi | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 17 | Neural ensemble dynamics underlying a long-te | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 18 | Cognitive-Load Theory: Methods to Manage Work | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 19 | Development of Spatial and Verbal Working Mem | 6/9 | STALE, DECAYED | — | 0 | 0 |
| 20 | Cephalopod Behavior: From Neural Plasticity t | 6/9 | STALE, DECAYED | — | 0 | 0 |

### 5.4 Branch Cataloging

**Clusters:** 4 total (2 emerging, 2 established)
**Structural mirrors:** 1

| Cluster | Size | Status | Avg Year |
|---------|------|--------|----------|
| Faster R-CNN: Towards Real-Time Object Detection w | 2192 | Established | 2017.9 |
| Data-driven biomarkers outperform theory-based bio | 2 | Emerging | 2023.5 |
| Neural mechanisms of divided feature-selective att | 2 | Established | 2021.0 |
| bendik-eeg-henriksen/nor-casehold (dataset) | 2 | Emerging | 2026.0 |

**Structural Mirrors:**

| Cluster A | Cluster B | Similarity |
|-----------|-----------|------------|
| Data-driven biomarkers outperform t | Neural mechanisms of divided featur | 1.00 |

### 5.5 Golden Token Pathfinding

**Path length:** 10
**Composite score:** 5.202
**Confidence:** HIGH
**Entropy nodes resolved:** 1

**Scoring weights:**

| Factor | Weight |
|--------|--------|
| Centrality | 0.25 |
| Recency | 0.25 |
| Entropy Resolution | 0.25 |
| Strategic Alignment | 0.25 |

**Full Path:**

| Step | Source | Year | Score | Gap Node |
|------|--------|------|-------|----------|
| 1 | Neural Correlates of Verbal Working Memory: An fMR | 2019 | 0.560 | YES |
| 2 | sajqavril/fmri-latents-private (dataset) | 2025 | 0.391 | — |
| 3 | RoboCOIN/Galaxea_R1_Lite_change_baai_into_brain (d | 2026 | 0.532 | — |
| 4 | gasparyanartur/things-eeg2 (dataset) | 2026 | 0.543 | — |
| 5 | braindecode/isruc-sleep (dataset) | 2026 | 0.531 | — |
| 6 | braindecode/bcic2020-3 (dataset) | 2026 | 0.530 | — |
| 7 | braindecode/faced (dataset) | 2026 | 0.529 | — |
| 8 | braindecode/chbmit (dataset) | 2026 | 0.529 | — |
| 9 | braindecode/physionet (dataset) | 2026 | 0.529 | — |
| 10 | braindecode/mdd_mumtaz2016 (dataset) | 2026 | 0.528 | — |

## 6. Supplementary Analyses

### 6.1 Co-Occurrence Patterns (Apriori)

**Frequent 1-itemsets:** 19
**Frequent 2-itemsets:** 60
**Frequent 3-itemsets:** 25
**Association rules:** 24

**Parameters:** min_support = 0.05, min_confidence = 0.60

**Top Frequent Patterns:**

| Pattern | Support |
|---------|---------|
| [Emerging] AND research in Neuroscience | 0.239 |
| using Neuroimaging AND research in Neuroscience | 0.237 |
| (Foundational work) AND research in Neuroscience | 0.208 |
| [Highly Cited] AND research in Neuroscience | 0.183 |
| [Emerging] AND using Neuroimaging | 0.177 |
| research in Computer Science AND research in Neuroscience | 0.168 |
| using Neuroimaging AND research in Computer Science | 0.167 |
| [Emerging] AND (Very Recent work) | 0.159 |
| [Highly Cited] AND research in Computer Science | 0.148 |
| studying Clinical AND research in Neuroscience | 0.142 |

**Top Association Rules (by lift):**

| Antecedent | Consequent | Support | Confidence | Lift |
|------------|-----------|---------|------------|------|
| topic:psychology | topic:medicine | 0.070 | 0.77 | 4.26 |
| topic:psychology | temporal:established | 0.056 | 0.62 | 2.59 |
| impact:highly_cited + topic:ne | temporal:foundational | 0.118 | 0.65 | 2.53 |
| temporal:foundational + topic: | impact:highly_cited | 0.057 | 0.68 | 2.50 |
| impact:highly_cited + topic:bi | temporal:foundational | 0.057 | 0.62 | 2.43 |
| temporal:foundational + topic: | impact:highly_cited | 0.064 | 0.66 | 2.40 |
| topic:education | impact:emerging | 0.072 | 0.87 | 2.01 |
| impact:highly_cited + temporal | topic:neuroscience | 0.118 | 0.94 | 1.91 |
| method:machine_learning | topic:computer science | 0.069 | 0.67 | 1.89 |
| temporal:foundational + topic: | topic:neuroscience | 0.089 | 0.91 | 1.84 |

### 6.2 Synchronicity Detection (SVW)

**Candidate pairs:** 66000
**High-tier pairs:** 1433
**Convergence events:** 1

**Parameters:** similarity threshold = 0.40, zero citation linkage required

**Convergence Events:**

| Event | Score | Groups | Time Window | Papers |
|-------|-------|--------|------------|--------|
| svw_001 | 0.153 | 1565 | 48y | 1565 |

### 6.3 Generated Hypotheses

**Total generated:** 5

**H01** [MEDIUM]
- **Hypothesis:** Re-examining 'Synaptic Plasticity Forms and Functions.' with contemporary methods will produce findings that substantially update or contradict current consensus in this area
- **Grounded in:** entropy gap node: Synaptic Plasticity Forms and Functions.
- **Novel because:** not yet connected to 'Medicine' despite logical dependency in prerequisite chain
- **Suggested method:** neuroimaging (co-occurs with similar gaps at 84% confidence in apriori patterns)
- **Gap node:** `97ab16d0e4a98f8275932f0a684e4fdcd70fb2d3` (severity: 6)
- **On golden path:** No

**H02** [MEDIUM]
- **Hypothesis:** Re-examining '20 years of the default mode network: A review and synthesis.' with contemporary methods will produce findings that substantially update or contradict current consensus in this area
- **Grounded in:** entropy gap node: 20 years of the default mode network: A review and synthe...
- **Novel because:** not yet connected to 'Medicine' despite logical dependency in prerequisite chain
- **Suggested method:** neuroimaging (co-occurs with similar gaps at 84% confidence in apriori patterns)
- **Gap node:** `74e9cad6e3d8b4187216aaaaa171d808e48d34f0` (severity: 6)
- **On golden path:** No

**H03** [MEDIUM]
- **Hypothesis:** Re-examining 'Review and Classification of Emotion Recognition Based on EEG Brain...' with contemporary methods will produce findings that substantially update or contradict current consensus in this area
- **Grounded in:** entropy gap node: Review and Classification of Emotion Recognition Based on...
- **Novel because:** not yet connected to 'Psychology' despite logical dependency in prerequisite chain
- **Suggested method:** neuroimaging (co-occurs with similar gaps at 84% confidence in apriori patterns)
- **Gap node:** `b067b33daddbd064592e12f2a4c1cfc300a2992e` (severity: 6)
- **On golden path:** No

**H04** [MEDIUM]
- **Hypothesis:** Re-examining 'From Cognitive Load Theory to Collaborative Cognitive Load Theory' with contemporary methods will produce findings that substantially update or contradict current consensus in this area
- **Grounded in:** entropy gap node: From Cognitive Load Theory to Collaborative Cognitive Loa...
- **Novel because:** not yet connected to 'Medicine' despite logical dependency in prerequisite chain
- **Suggested method:** neuroimaging (co-occurs with similar gaps at 84% confidence in apriori patterns)
- **Gap node:** `a7dacb4598eb77ea3d84dc2c2c2603de49d6acd9` (severity: 6)
- **On golden path:** No

**H05** [MEDIUM]
- **Hypothesis:** Re-examining 'Casting a Wide Net: Role of Perineuronal Nets in Neural Plasticity' with contemporary methods will produce findings that substantially update or contradict current consensus in this area
- **Grounded in:** entropy gap node: Casting a Wide Net: Role of Perineuronal Nets in Neural P...
- **Novel because:** not yet connected to 'Psychology' despite logical dependency in prerequisite chain
- **Suggested method:** neuroimaging (co-occurs with similar gaps at 84% confidence in apriori patterns)
- **Gap node:** `b49714edbef46eb488b64b25f061c3f37da86742` (severity: 6)
- **On golden path:** No


## 8. Limitations

1. **QEB encoding**: When using TF-IDF + SVD fallback, semantic nuance may be lost for polysemous terms and domain-specific jargon. Sentence-BERT (all-MiniLM-L6-v2) mitigates this but is limited to 384-dimensional embeddings.

2. **Source coverage**: The Semantic Scholar API provides a subset of published research. Important works outside this index, preprints on non-indexed platforms, and unpublished research are not captured.

3. **Temporal decay calibration**: Parameters (λ=0.12, floor=0.55) are calibrated for general academic literature. Domains with different publication rhythms (e.g., fast-moving ML vs. slow clinical trials) may need domain-specific calibration.

4. **Greedy pathfinding**: The golden token algorithm uses greedy selection at each step. This does not guarantee globally optimal trajectories. Future work: backtracking or beam search.

5. **Abstract-only synthesis**: Gap-bridging proposals are synthesized from abstract text only. Full-text access would enable deeper, more specific synthesis.

6. **Citation completeness**: Citation edges depend on reference list coverage in the Semantic Scholar graph. Missing citations can create false ORPHAN detections.

7. **CONTRADICTS edges**: Cosine similarity (whether from TF-IDF or Sentence-BERT) cannot detect semantic contradiction directly. CONTRADICTS edges are inferred from structural patterns, not content analysis.

## 9. Reproducibility

### 9.1 Engine Configuration

| Parameter | Value |
|-----------|-------|
| Engine version | 0.3.0-prototype |
| Total sources | 2254 |
| Timestamp | 2026-04-05T20:36:37.385169 |
| Embedding backend | tfidf |
| Embedding dim | 256 |
| Perturbation σ | 0.1 |
| Temporal decay λ | 0.12 |
| Temporal decay floor | 0.55 |
| Backward max depth | 20 |
| Forward max depth | 15 |
| Forward confidence floor | 0.1 |
| Concept drift threshold | 0.4 |
| Forgotten ancestor days | 60 |
| Staleness threshold days | 30 |
| Min cluster size | 2 |
| Structural mirror threshold | 0.4 |
| Apriori min support | 0.05 |
| Apriori min confidence | 0.60 |
| SVW similarity threshold | 0.40 |
| Golden token path min/max | 3–10 |

### 9.2 Patent References

- QECO: Three-Signal Adaptive Confidence (Module 1)
- QECO: Epistemic Grounding Score (Module 1b)
- QECO: Co-Occurrence Pattern Discovery (Module 2, Apriori)
- CTE: All 5 Operations
- CTE: Domain-Tagged Concept Graph + Temporal Decay
- SVW: Synchronicity Weaver
- Generation: Hypothesis Production
- Cross-Domain Bridge Detection

### 9.3 How to Replicate

1. Install dependencies: `pip install -r requirements.txt`
2. Set API keys: `SEMANTIC_SCHOLAR_API_KEY`, `ANTHROPIC_API_KEY` (optional, for auto-builder)
3. Run: `python main.py <domain_key>` for single-domain, or `python main.py <domain1> <domain2> --unified` for cross-domain
4. Reports output to `data/<domain_key>/`

*DCFN is built on patented technology by Living Eden Frameworks LLC: CTE Patent 64/002,205, QECO Patent 63/993,979, LPA Patent 64/023,988.*